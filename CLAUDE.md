...
- **Aerodrome CL factory ABI difere do V3**: usa `getPool(address,address,int24 tickSpacing)` em vez de `uint24 fee` — function selectors diferentes. `EvmScanner` e `EvmClReader` selecionam ABI automaticamente via `TICK_SPACING_FACTORY_DEXES` set. O campo `fee` retornado pelo PM da Aerodrome é na verdade o `tickSpacing` (ex: 1, 100, 200).
